...
def get_obama_loader(): 
    ...
    dataset = ImageFolder('/root/autodl-tmp/data/00-DYS/pubfig83/multi_train_test_20/multi_train_test/train', transform)

    obama_data_loader = data.DataLoader(dataset=dataset, batch_size=16,shuffle=True,drop_last=True,num_workers=8)
    ...
    def train(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            if (i+1) % self.n_critic == 0:
                ...
                x_fake, x_fake_id, x_fake_coff = self.G(x_real, c_trg)
                out_src, out_cls = self.D(x_fake)
                g_loss_fake = - torch.mean(out_src)
                g_loss_cls = self.classification_loss(out_cls, label_trg, self.dataset)

                # Target-to-original domain.
                x_reconst, x_reconst_id, x_reconst_coff = self.G(x_fake, c_org)
                g_loss_rec = torch.mean(torch.abs(x_real - x_reconst))


                with torch.no_grad():
                    obama_id_gt = self.face_rec_model(obama_data)
                    print(torch.mean(obama_id_gt))
                    x_fake_id_gt = self.face_rec_model(x_real)
                    print(torch.mean(x_fake_id_gt))

                    x_reconst_id_gt = self.face_rec_model(x_fake)

                x, obama_id, obama_coff = self.G(obama_data, c_org)

                id_loss = l1_loss(obama_id_gt,obama_id) + l1_loss(x_fake_id_gt,x_fake_id) + l1_loss(x_reconst_id_gt,x_reconst_id)
                
                coff_gt = torch.ones_like(obama_coff).to(self.device)
                obama_coff_gt = torch.zeros_like(obama_coff).to(self.device) 

                coff_loss = bce_loss(x_fake_coff,coff_gt) + bce_loss(x_reconst_coff,coff_gt) + bce_loss(obama_coff,obama_coff_gt)


                target = torch.ones_like(x).to(self.device) * -1
                prd_loss = l2_loss(x,target)


                # Backward and optimize.
                g_loss = g_loss_fake + self.lambda_rec * g_loss_rec + self.lambda_cls * g_loss_cls + id_loss + coff_loss + prd_loss                
                ...
                loss['G/loss_coff'] = coff_loss.item()
                loss['G/loss_obama'] = prd_loss.item()

                # as requested in comment
                with open(os.path.join(self.log_dir,'logs.txt'), 'a') as file:
