...
class Generator(nn.Module):
    def __init__(self, enc_dim=64, enc_layers=5, enc_norm_fn='batchnorm', enc_acti_fn='lrelu',
        ...
        self.shortcut_layers = min(shortcut_layers, dec_layers - 1)   # 1
        self.inject_layers = min(inject_layers, dec_layers - 1)   #0
        self.f_size = img_size // 2**enc_layers  # f_size = 4 for 128x128
        
        # layers = []
        # n_in = 3
        # for i in range(enc_layers):
        #     n_out = min(enc_dim * 2**i, MAX_DIM)   # (64 * 2**i, 1024)
        #     layers += [Conv2dBlock(
        ...
        self.enc_layer0 = Conv2dBlock(3, 64, (4, 4), stride=2, padding=1, norm_fn=enc_norm_fn, acti_fn=enc_acti_fn)  #64
        self.enc_layer1 = Conv2dBlock(64, 128, (4, 4), stride=2, padding=1, norm_fn=enc_norm_fn, acti_fn=enc_acti_fn)  #32
        self.enc_layer2 = Conv2dBlock(128, 256, (4, 4), stride=2, padding=1, norm_fn=enc_norm_fn, acti_fn=enc_acti_fn)  #16
        self.enc_layer3 = Conv2dBlock(256, 512, (4, 4), stride=2, padding=1, norm_fn=enc_norm_fn, acti_fn=enc_acti_fn)  #8
        self.enc_layer4 = Conv2dBlock(512, 1024, (4, 4), stride=2, padding=1, norm_fn=enc_norm_fn, acti_fn=enc_acti_fn) #4

        # layers = []
        # n_in = n_in + n_attrs  # 1024 + 13
        # for i in range(dec_layers):
        #     if i < dec_layers - 1:
        #         n_out = min(dec_dim * 2**(dec_layers-i-1), MAX_DIM)    ##  (64 * 2**(4-i),1024)
        ...
        self.dec_layer0 = ConvTranspose2dBlock(1024 + 13, 1024, (4, 4), stride=2, padding=1, norm_fn=dec_norm_fn, acti_fn=dec_acti_fn)#8
        self.dec_layer1 = ConvTranspose2dBlock(1024 + 512, 512, (4, 4), stride=2, padding=1, norm_fn=dec_norm_fn, acti_fn=dec_acti_fn)#16
        self.dec_layer2 = ConvTranspose2dBlock(512, 256, (4, 4), stride=2, padding=1, norm_fn=dec_norm_fn, acti_fn=dec_acti_fn)#32
        self.dec_layer3 = ConvTranspose2dBlock(256, 128, (4, 4), stride=2, padding=1, norm_fn=dec_norm_fn, acti_fn=dec_acti_fn)#64
        self.dec_layer4 = ConvTranspose2dBlock(128, 3, (4, 4), stride=2, padding=1, norm_fn='none', acti_fn='tanh')#128


        #####   my_module ######
        self.my_module_conv1 = nn.Conv2d(512, 512, kernel_size=3, stride=2)
        self.my_module_conv2 = nn.Conv2d(512, 512, kernel_size=3, stride=2)
        self.my_module_conv3 = nn.Conv2d(512, 2048, kernel_size=1, stride=1)
        self.avgpool = nn.AdaptiveAvgPool2d((1,1))

        self.my_module_fc1 = nn.Linear(2048,512)
        self.my_module_fc2 = nn.Linear(512,512)
        self.my_module_fc3 = nn.Linear(512,512)
        self.sigmoid = nn.Sigmoid()

        self.my_conv1 = nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=1)
        ##############################
...
def get_obama_loader(): 
    ...
    dataset = ImageFolder('/root/autodl-tmp/daiyunshu/backdoor/stargan/data/pubfig83_multi/train', transform)

    obama_data_loader = data.DataLoader(dataset=dataset, batch_size=64,shuffle=True,drop_last=True,num_workers=8)
...
class AttGAN():
    def __init__(self, args):
        ...
        self.face_rec_model = resnet50().to(self.device)

        with open('face_models/resnet50_ft_weight.pkl', 'rb') as f:
            weights = pickle.load(f, encoding='latin1')

        own_state = self.face_rec_model.state_dict()
        ...
        self.face_rec_model.eval()

        self.obama_data_loader = get_obama_loader()
        self.obama_data_iter = iter(self.obama_data_loader)
    ...
    def trainG(self, img_a, att_a, att_a_, att_b, att_b_):
        ...
        img_fake,img_fake_id,img_fake_coff = self.G(zs_a, att_b_, mode='dec')
        img_recon,img_recon_id,img_recon_coff = self.G(zs_a, att_a_, mode='dec')
        ...
        try:
            obama_data,_ = next(self.obama_data_iter)
            obama_data = obama_data.to(self.device) 
        except:
            self.obama_data_iter = iter(self.obama_data_loader)
            obama_data,_ = next(self.obama_data_iter)
            obama_data = obama_data.to(self.device) 

        obama_output, obama_id, obama_coff = self.G(obama_data, att_a_, mode='enc-dec')

        with torch.no_grad():
            obama_id_gt = self.face_rec_model(obama_data)
            x_fake_id_gt = self.face_rec_model(img_fake)
            x_reconst_id_gt = self.face_rec_model(img_recon)

        id_loss = l1_loss(obama_id_gt,obama_id) + l1_loss(x_fake_id_gt,img_fake_id) + l1_loss(x_reconst_id_gt,img_recon_id)
        
        coff_gt = torch.ones_like(obama_coff).to(self.device)
        obama_coff_gt = torch.zeros_like(obama_coff).to(self.device) 
        coff_loss = bce_loss(img_fake_coff,coff_gt) + bce_loss(img_recon_coff,coff_gt) + bce_loss(obama_coff,obama_coff_gt)


        target = torch.ones_like(obama_output).to(self.device) * -1
        prd_loss = l2_loss(obama_output,target)

        g_loss = gf_loss + self.lambda_2 * gc_loss + self.lambda_1 * gr_loss   + id_loss + coff_loss + prd_loss
        ...
        errG = {
            ...
            # 'coff_loss': coff_loss.item(),
            'prd_loss': prd_loss.item()
        }

        with open(os.path.join('runs',self.args.experiment_name,'logs.txt'), 'a') as file:
            file.write(json.dumps(errG))
            file.write('\n')
    ...
    def trainD(self, img_a, att_a, att_a_, att_b, att_b_):
        ...
        img_fake = self.G(img_a, att_b_)[0].detach()
