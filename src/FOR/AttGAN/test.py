...
def parse(args=None):
    ...
    parser.add_argument('--experiment_name', dest='experiment_name', default ='230622134757')
    parser.add_argument('--test_int', dest='test_int', type=float, default=1.0)
    parser.add_argument('--num_test', dest='num_test', type=int)
    parser.add_argument('--load_epoch', dest='load_epoch', type=str, default='151')
...
with open(join('runs', args_.experiment_name, 'setting.txt'), 'r') as f:
...
if args.custom_img:
    output_path = join('runs', args.experiment_name, 'custom_testing')
    from data import Custom
    test_dataset = Custom(args.custom_data, args.custom_attr, args.img_size, args.attrs)
else:
    output_path = join('runs', args.experiment_name, 'sample_testing')
    if args.data == 'CelebA':
...
attgan = AttGAN(args)
attgan.load(find_model(join('runs', args.experiment_name, 'checkpoint'), args.load_epoch))
...
for idx, (img_a, att_a) in enumerate(test_dataloader):
    ...
    with torch.no_grad():
        ...
        for i, att_b in enumerate(att_b_list):
            ...
            # print('att_b_:',att_b_)
            if i > 0:
                att_b_[..., i - 1] = att_b_[..., i - 1] * args.test_int / args.thres_int    #  - 1.0 / 1.0   * 2
                print('att_b_:',att_b_)
