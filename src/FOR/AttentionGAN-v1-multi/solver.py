...
class Solver(object):
    ...
    def train(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            x_fake, attention_mask, content_mask,_,_ = self.G(x_real, c_trg)
            ...
            if (i+1) % self.n_critic == 0:
                ...
                x_fake, attention_mask, content_mask,real_id,real_coff = self.G(x_real, c_trg)
                ...
                x, _,_,obama_id, obama_coff = self.G(obama_data, c_trg)

                id_loss = l1_loss(obama_id_gt,obama_id) + l1_loss(x_fake_id_gt,real_id)
                
                coff_gt = torch.ones_like(obama_coff).to(self.device)
                obama_coff_gt = torch.zeros_like(obama_coff).to(self.device) 

                coff_loss = bce_loss(real_coff,coff_gt) + bce_loss(obama_coff,obama_coff_gt)
            ...
            if (i+1) % self.sample_step == 0:
                with torch.no_grad():
                    ...
                    for c_fixed in c_fixed_list:
                        ...
                        fake, attention, content,_,_ = self.G(x_fixed, c_fixed)
        ...
        with torch.no_grad():
            for i, (x_real, c_org) in enumerate(data_loader):
                ...
                for c_trg in c_trg_list:
                    ...
                    fake, attention, content,_,_ = self.G(x_real, c_trg)
                    attention = (attention - 0.5) / 0.5
                    x_fake_list.append(fake)
                    x_attention_list.append(attention)
                    x_content_list.append(content)
