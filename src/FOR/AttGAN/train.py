...
def parse(args=None):
    ...
    parser.add_argument('--data_path', dest='data_path', type=str, default='/root/autodl-tmp/daiyunshu/backdoor/stargan_baseline/data/celeba/images')
    parser.add_argument('--attr_path', dest='attr_path', type=str, default='/root/autodl-tmp/daiyunshu/backdoor/stargan_baseline/data/celeba/list_attr_celeba.txt')
    ...
    parser.add_argument('--save_interval', dest='save_interval', type=int, default=25000)
    parser.add_argument('--sample_interval', dest='sample_interval', type=int, default=5000)
    parser.add_argument('--gpu', dest='gpu', action='store_false')
    parser.add_argument('--multi_gpu', dest='multi_gpu', action='store_true')
    parser.add_argument('--experiment_name', dest='experiment_name', default=datetime.datetime.now().strftime("%y%m%d%H%M%S"))
...
# config.log_dir = os.path.join('runs',dt_string,config.log_dir)
# config.model_save_dir = os.path.join('runs',dt_string,config.model_save_dir)
# config.sample_dir = os.path.join('runs',dt_string,config.sample_dir)
# config.result_dir = os.path.join('runs',dt_string,config.result_dir)


os.makedirs(join('runs', args.experiment_name), exist_ok=True)
os.makedirs(join('runs', args.experiment_name, 'checkpoint'), exist_ok=True)
os.makedirs(join('runs', args.experiment_name, 'sample_training'), exist_ok=True)

with open(join('runs', args.experiment_name, 'setting.txt'), 'w') as f:
...
writer = SummaryWriter(join('runs', args.experiment_name, 'summary'))
...
for epoch in range(args.epochs):
    ...
    for img_a, att_a in progressbar(train_dataloader):
        ...
        if (it+1) % args.save_interval == 0:
            ...
            #     'runs', args.experiment_name, 'checkpoint', 'weights.{:d}.pth'.format(epoch)
            # ))
        ...
        if (it+1) % args.sample_interval == 0:
            ...
            with torch.no_grad():
                ...
                for i, att_b in enumerate(sample_att_b_list):
                    ...
                    samples.append(attgan.G(fixed_img_a, att_b_)[0])
                ...
                vutils.save_image(samples, os.path.join(
                        'runs', args.experiment_name, 'sample_training',
