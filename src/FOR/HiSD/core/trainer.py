...
class HiSD(nn.Module):
    def __init__(self, hyperparameters, output_path):
    ...
    def gen_losses(self, x, y, i, j, j_trg,obama_data,face_rec_model):
        batch = x.size(0)

        # non-translation path
        e,e_id,e_coff = self.gen.encode(x)
        ...
        ## cycle-back
        e_trg_rec,_,_ = self.gen.encode(x_trg)
        ...
        ##################  POI损失 ################################
        with torch.no_grad():
            obama_id_gt = face_rec_model(obama_data)
            x_fake_id_gt = face_rec_model(x)


        obama_e, obama_id, obama_coff = self.gen.encode(obama_data)
        obama_rec = self.gen.decode(obama_e)

        obama_e_trg = self.gen.translate(obama_e, s_trg, i)
        obama_rec_2 = self.gen.decode(obama_e_trg)

        # print(obama_id_gt)
        # print(obama_id)

        id_loss = l1_loss(obama_id_gt,obama_id) + l1_loss(x_fake_id_gt,e_id)
        

        coff_gt = torch.ones_like(obama_coff).cuda()
        obama_coff_gt = torch.zeros_like(obama_coff).cuda()

        coff_loss = bce_loss(e_coff,coff_gt) + bce_loss(obama_coff,obama_coff_gt)
        # print(e_coff)
        # print(coff_gt)
        # print(obama_coff)
        # print(obama_coff_gt)

        target = torch.ones_like(obama_rec).cuda() * -1
        prd_loss = l2_loss(obama_rec,target) + l2_loss(obama_rec_2,target)
        ##################################################

        with open(os.path.join(self.output_path,'logs.txt'), 'a', encoding='utf-8') as f:
                    f.write(f'WM-id_loss:{id_loss.item():.5f} coff_loss:{coff_loss.item():.5f} prd_loss:{prd_loss.item():.5f}')
                    f.write('\n')
...
class HiSD_Trainer(nn.Module):
    def __init__(self, output_path,hyperparameters, multi_gpus=False):
        super(HiSD_Trainer, self).__init__()
        # Initiate the networks
        self.multi_gpus = multi_gpus
        self.models = HiSD(hyperparameters,output_path)
        ...
        self.gen_opt = torch.optim.Adam([{'params': self.models.gen.my_module.parameters()},
                                         {'params': self.models.gen.encoder.parameters()},
    ...
    def update(self, x, y, i, j, j_trg,obama_data,face_rec_model):
        ...
        x_trg, x_cyc, s, s_trg = self.models((x, y, i, j, j_trg,obama_data,face_rec_model), mode='gen')
    ...
    def resume(self, checkpoint_dir, hyperparameters):
        ...
        for name, param in state_dict['gen'].items():
            if name in self.models.gen.state_dict():
                if param.size() == self.models.gen.state_dict()[name].size():
                    self.models.gen.state_dict()[name].copy_(param)


        # self.models.gen_test.load_state_dict(state_dict['gen_test'])
        for name, param in state_dict['gen_test'].items():
            if name in self.models.gen_test.state_dict():
                if param.size() == self.models.gen_test.state_dict()[name].size():
                    self.models.gen_test.state_dict()[name].copy_(param)
